...
// Added support for Qt's QFile, so we can use resource files
// to avoid path problems
std::string LoadObj(std::vector<shape_t> &shapes,
                    std::vector<material_t> &materials, // [output]
                    QFile &file) {
  std::string basePath;
  MaterialFileReader readMatFn(basePath);
  ...
  qint64 maxchars = 8192;             // Alloc enough size.
  std::vector<char> buf(maxchars); // Alloc enough size.
  while (!in.atEnd()) {
    QString qstr_line = in.readLine(maxchars);

    std::string linebuf = qstr_line.toStdString();
